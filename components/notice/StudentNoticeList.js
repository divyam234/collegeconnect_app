...
const AttendanceCard = (props) => {
    ...
    return (
      ...
      <Card>
        ...
        onPress={() => props.navigation.navigate('StudentNoticeView',{item})} >View</Button>
        </Card.Actions>
       </Card>
      </Surface>
    )
  ...
  class StudentNoticeList extends PureComponent {
    state = {
      data: [],
      refreshing: false
    };
  ...
  async componentDidMount() {
     let {loadnoticedata,details,type}=this.props
     await loadnoticedata(type,details);
...
const mapStateToProps=(state)=>{
   return{
     loading:state.student.noticelist.loading,
     list:state.student.noticelist.list,
     details:state.student.profileDetails,
     email:state.auth.id,
...
export default connect(mapStateToProps,mapDispatchToProps)(StudentNoticeList);
